...
def upload_file2(request):
    if request.method == "POST":  # Проверяем, был ли отправлен POST запрос
        ...
        )  # Инициализируем форму загрузки файлов с данными из запроса
        if form.is_valid():  # Проверяем валидность данных формы
            file = request.FILES["file"]  # Получаем загруженный файл из формы
            uploaded_by = (
                request.user if request.user.is_authenticated else None
            )  # Определяем пользователя, загрузившего файл (если он авторизован)
            fp = CustomFiles(
                file=form.cleaned_data["file"],
                file_name=file.name,
                uploaded_by=uploaded_by,
            )  # Создаем экземпляр CustomFiles для сохранения файла в базе данных
